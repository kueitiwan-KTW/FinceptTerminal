...
# ── 解析 Qt 路徑 ──────────────────────────────────────────────────────────────
QT_ARCH="$(cat /etc/qt_arch)"
QT_PREFIX="${QT_ROOT:-/opt/Qt}/6.8.3/${QT_ARCH}"
...
# 確認翻譯檔存在
if [ -f "/app/translations/fincept_zh_TW.qm" ] || [ -f "/app/resources/translations/fincept_zh_TW.qm" ]; then
