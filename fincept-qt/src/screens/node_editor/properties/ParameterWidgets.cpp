...
QWidget* ParameterWidgetFactory::create(const ParamDef& param, const QJsonValue& current_value,
    ...
    if (param.type == "string" || param.type == "expression") {
        ...
        auto populate = [combo, allowed_exts](const QString& select_path = {}) {
            ...
            combo->clear();
            combo->addItem(QObject::tr("— select file —"), QString());
        ...
        auto* import_btn = new QPushButton(QObject::tr("+ Import"));
        ...
        auto populate_agents = [combo, current_value]() {
            ...
            combo->clear();
            combo->addItem(QObject::tr("— select agent —"), QString());
        ...
        if (svc.cached_agent_count() > 0) {
            ...
            combo->addItem(QObject::tr("Loading agents…"), QString());
            trigger_discovery();
        }
        ...
        combo->setStyleSheet(
            QString("QComboBox { %1 }"
                    ...
                    "  font-family:Consolas; }")
                .arg(input_style(), ui::colors::BORDER_MED(), ui::colors::BG_HOVER(), ui::colors::TEXT_PRIMARY()));
        combo->addItem(QObject::tr("— agent default —"), QString());
        ...
        auto* hint = new QLabel(QObject::tr("Leave blank to use the LLM assigned to the agent in Agent Config"));
        ...
        auto populate_tools = [combo, current_value]() {
            ...
            combo->clear();
            combo->addItem(QObject::tr("— select tool —"), QString());
            ...
            for (const auto& t : tools) {
                if (t.server_name != last_server) {
                    ...
                    combo->addItem(QObject::tr("── ") + t.server_name + " ──", QString());
        ...
        combo->addItem(QObject::tr("Loading tools…"), QString());
        ...
        auto* hint = new QLabel(QObject::tr("All Fincept internal tools. Input JSON flows in as arguments."));
        hint->setStyleSheet(QString("color:%1; font-family:Consolas; font-size:10px;").arg(ui::colors::TEXT_TERTIARY()));
        hint->setWordWrap(true);
        layout->addWidget(hint);
