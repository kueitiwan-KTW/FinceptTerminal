...
void NodePropertiesPanel::build_ui() {
    ...
    auto* empty_label = new QLabel(tr("Select a node\nto edit properties"));
...
void NodePropertiesPanel::build_editor(const NodeDef& node, const NodeTypeDef& type_def) {
    ...
    auto* title = new QLabel(tr("PROPERTIES"));
    ...
    if (!type_def.parameters.isEmpty()) {
        auto* param_header = new QLabel(tr("PARAMETERS"));
    ...
    auto* settings_header = new QLabel(tr("SETTINGS"));
