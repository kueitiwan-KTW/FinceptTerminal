...
QWidget* AkShareScreen::create_header() {
    ...
    auto* title = new QLabel(tr("AKSHARE DATA EXPLORER"));
    title->setObjectName("akHeaderTitle");
    auto* sub = new QLabel(tr("1000+ CHINESE & GLOBAL FINANCIAL DATA ENDPOINTS"));
    ...
    auto* badge = new QLabel(tr("FREE API"));
...
QWidget* AkShareScreen::create_endpoint_panel() {
    ...
    endpoint_count_ = new QLabel(tr("0 endpoints"));
    ...
    // Empty state
    auto* empty = new QLabel(tr("Select a data source above\nto load available endpoints"));
...
void AkShareScreen::on_source_clicked(int index) {
    ...
    status_source_->setText(tr("SOURCE: ") + sources_[index].name.toUpper());
