...
QString DockScreenRouter::title_for_id(const QString& id) {
    // 面板標題 — 透過 QCoreApplication::translate 讓翻譯系統可覆蓋
    // context 設為 "fincept::DockScreenRouter" 以對齊 .ts 翻譯檔
    auto t = [](const char* src) {
        return QCoreApplication::translate("fincept::DockScreenRouter", src);
    };
    ...
    return it != keys.end() ? t(it.value()) : id;
}
